...
│     ↓                                                                        │
│  2. npx lingui extract --clean  →  掃描所有程式碼，更新 .po 檔案              │
│     ↓                                                                        │
│  3. 所有語言的 .po 檔案都會被更新（新增 msgid、調整引用位置）                   │
│     ↓                                                                        │
│  4. 人工編輯 zh-TW.po  →  填入 msgstr 翻譯                                    │
│     ↓                                                                        │
│  5. npx lingui compile  →  編譯 .po 為 .ts                                   │
│     ↓                                                                        │
│  6. 所有語言的 generated/*.ts 都會被更新                                      │
│     ↓                                                                        │
│  7. 程式執行時載入 zh-TW.ts 顯示翻譯                                          │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 為什麼所有語言的檔案都會被更新？

執行 `lingui extract` 時，會同步更新**所有語言**的 `.po` 檔案結構：
- 新增新的 `msgid`
- 移除不再使用的字串
- 調整引用位置（`#:` 註解）

即使你只修改了一個文案，所有語言的 `.po` 和 `.ts` 檔案都會有變更，這是正常的。
