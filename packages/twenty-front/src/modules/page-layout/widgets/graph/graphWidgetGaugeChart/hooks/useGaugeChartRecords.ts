...
import { type GaugeChartConfiguration } from '~/generated/graphql';

const GAUGE_CHART_TOOLTIP_RECORDS_LIMIT = 50;

type UseGaugeChartRecordsProps = {
...
type UseGaugeChartRecordsResult = {
  records: ChartRecord[];
  totalCount: number;
  loading: boolean;
};
...
const globalGaugeChartCache = new Map<string, CachedChartData>();

export const useGaugeChartRecords = ({
  objectMetadataItemId,
  configuration,
  enabled = true,
}: UseGaugeChartRecordsProps): UseGaugeChartRecordsResult => {
  ...
  const { records, loading, totalCount } = useFindManyRecords({
    ...
    limit: GAUGE_CHART_TOOLTIP_RECORDS_LIMIT,
  ...
  const cacheKey = `gauge-${objectMetadataItemId}`;
  ...
  useEffect(() => {
    if (!loading && transformedRecords.length > 0) {
      ...
      globalGaugeChartCache.set(cacheKey, newData);
      lastSuccessfulDataRef.current = newData;
    }
  }, [loading, transformedRecords, totalCount, cacheKey]);

  const cachedData = globalGaugeChartCache.get(cacheKey);
