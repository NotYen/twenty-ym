...
# 運行數據庫遷移
if npx nx run twenty-server:database:migrate:prod > ${LOG_DIR}/ycrm_migration.log 2>&1; then
    echo "   ✅ 數據庫遷移完成"

    # 顯示執行了哪些遷移
    if grep -q "migration" ${LOG_DIR}/ycrm_migration.log 2>/dev/null; then
        echo "   📋 已執行的遷移："
        grep -i "migration" ${LOG_DIR}/ycrm_migration.log | tail -5 || echo "      （數據庫已是最新版本）"
    else
    ...
    echo ""
    echo "📋 錯誤日誌："
    cat ${LOG_DIR}/ycrm_migration.log
    echo ""
    echo "💡 可能原因："
    ...
    echo "   - 查看詳細日誌: cat ${LOG_DIR}/ycrm_migration.log"
    echo ""
    exit 1
fi
echo ""
...
# 清理舊的日誌
if [ -f "ycrm.log" ]; then
    mv ycrm.log "ycrm.log.$(date +%Y%m%d_%H%M%S).bak" 2>/dev/null || true
fi
...
if [ "$NODE_ENV" = "production" ]; then
    ...
    # 2. 啟動後端（production）
    echo "   🔧 啟動後端服務..."
    nohup bash -c "cd packages/twenty-server && node dist/src/main.js" > ${LOG_DIR}/ycrm_backend.log 2>&1 &

    # 3. 啟動前端（使用 serve）
    echo "   🌐 啟動前端服務..."
    nohup npx serve packages/twenty-front/build -l ${FRONTEND_PORT} -s > ${LOG_DIR}/ycrm_frontend.log 2>&1 &

    echo "   📝 日誌文件：${LOG_DIR}/ycrm_backend.log, ${LOG_DIR}/ycrm_frontend.log"
else
    echo "   🔧 Development 模式：使用熱重載"

    # Development 模式：使用熱重載
    nohup bash -c "npx nx run-many -t start -p twenty-server twenty-front" > ${LOG_DIR}/ycrm.log 2>&1 &
...
if npx nx run twenty-server:command workspace:sync-metadata >> ${LOG_DIR}/ycrm.log 2>&1; then
...
sleep 5
if npx nx run twenty-server:command cron:register:all >> ${LOG_DIR}/ycrm.log 2>&1; then
...
if [ "$NODE_ENV" = "production" ]; then
    ...
    nohup bash -c "cd packages/twenty-server && node dist/src/queue-worker/queue-worker.js" > ${LOG_DIR}/ycrm_worker.log 2>&1 &
    WORKER_PID=$!
    echo "   📝 Worker 日誌文件：${LOG_DIR}/ycrm_worker.log"
else
    echo "   🔧 Development 模式：使用 nx 運行 Worker"
    nohup bash -c "npx nx run twenty-server:worker" >> ${LOG_DIR}/ycrm.log 2>&1 &
    WORKER_PID=$!
    echo "   📝 Worker 日誌文件：ycrm.log"
fi
...
echo ""
echo "📝 查看日誌: tail -f ${LOG_DIR}/ycrm.log"
echo "🛑 停止服務: cd docker/dev-flow/local_build_for_mac && ./stop_all_services.sh"
echo ""
echo "💡 提示："
